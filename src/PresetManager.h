...
{
    std::string name;
    float delayMs     = paramInfo[pParamDelay].defaultVal;
    float rate        = paramInfo[pParamRate].defaultVal;
    float depth       = paramInfo[pParamDepth].defaultVal;
    float feedback    = paramInfo[pParamFeedback].defaultVal;
...
{
public:
    explicit PresetManager(ClassicFlangerUI* ui);
